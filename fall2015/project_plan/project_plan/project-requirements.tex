...
2.1	Environmental impacts and or concerns. Casing required, POE.

2.2	Paragraph 2, TCP handshake. Is it SSH handshake? Are we going to actually sandbox data for capture? 
	The Splunk server is on the parent network. It's a parser for various log formats which auto identifies logs that come in. We're
	reporting JSON that's structured in the way that we identified during our meeting.

2.2.1	Disposition of this section. Since we've resolved this since last completed.
}
